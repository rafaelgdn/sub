...
parser.add_argument("-r", "--range", type=int, help="Search type", default=15)
    ...
    for creator_page_link in creators_page_links:
        try:
                ...
                "name": (
                    await (
                        await driver.find_element(
                            By.CSS_SELECTOR,
                            'div[role="dialog"] + script + div > div > div h3',
                        )
                    ).text
                ).replace("\xa0", ""),
                "description": (
                    await (
                        await driver.find_element(
                            By.CSS_SELECTOR,
                            'div[role="dialog"] + script + div > div > div > div + div > div > span',
                        )
                    ).text
                ).replace("\n", " "),
            ...
            response = hrequests.get(
                f"https://{domain}/api/v1/post/search?query={quote(args.keyword)}&focusedPublicationId={publication_id}&page=0&numberFocused=10",
                headers=headers,
            )
